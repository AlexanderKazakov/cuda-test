...
complex<double> hammaM (const double &index) {
	double x = leftCorner + index * h;
	return ( omega - complex<double>(0, 1) * sigma(x) ) / ( omega * c(x)*c(x) );
}

complex<double> hammaK(const double &index) {
	double x = leftCorner + index * h;
	return ( omega ) / ( omega - complex<double>(0, 1) * sigma(x) );
}

complex<double> M(const int i, const int j) {
	if (i == j) {
...
complex<double> K(const int i, const int j) {
	if (i == j) {
		if (i == N) 
			return 1.0 / h * hammaK(N - 0.5);
		else
			return 1.0 / h * (hammaK(i - 0.5) + hammaK(i + 0.5));
	} else {
		return - 1.0 / h * hammaK((i > j ? i : j) - 0.5);
	}
}
...
complex<double> f(const int i) {
	if (i == 0) return omega*omega * h / 6 * hammaM(0.5) + 1.0 / h;
	else        return 0;
}
...
void reductionMethod(complex<double> (&_A)(const int, const int), 
	...
	// Checking the matrix on diagonal dominance
//	for(int i = 0; i < N; i++) {
//		double diff = abs(b[i]) - abs(a[i]) - abs(c[i]);
//		if (diff < 0) {
//			a[i] += diff/3.9; b[i] -= diff/1.9; c[i] += diff/3.9;
//		}
//	}
